...
public class ImageKit {
	...
	public static void saveWithQuality(BufferedImage im, float quality, String outputImageFile) {
		...
		/*	JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(newImage);
			...
			saveAsJPEG(500, im, quality, newImage);
			/* 近JPEG编码 */
			// newImage.close();
		} catch (Exception e) {
			throw new RuntimeException(e);
	 ...
	 * 上传图像
	 */
	public static RetKit uploadAvatar1(UploadFile uf,String folder) {
		if (uf == null) {
		...
		try {
			...
			String avatarUrl = "/upload/"+folder+"/";
			...
			avatarUrl = avatarUrl +  sysTime + extName;
			saveFile = saveFile +  sysTime + extName;
			zoom(500, uf.getFile(), saveFile);
			return RetKit.ok("url", avatarUrl.substring(1));
		}
		catch (Exception e) {
	 ...
	 * @return
	 */
	public static RetKit uploadAvatarUnZip1(UploadFile uf,String folder) {
		if (uf == null) {
			return RetKit.fail("msg", "上传文件UploadFile对象不能为null");
		}
		try {
			...
			long sysTime = System.currentTimeMillis();
			avatarUrl = avatarUrl +  sysTime +"." + getExtName(uf.getFileName());
			saveFile = saveFile +  sysTime +"." + getExtName(uf.getFileName());
			FileUtil.move(uf.getFile(), new File(saveFile), true);
			return RetKit.ok("url", avatarUrl.substring(1));
		}
		catch (Exception e) {
			return RetKit.fail("msg", e.getMessage());
	 ...
	 * @param url
	 */
	public static void deleteAvatar(String url){
		File oddAvatar = new File(PathKit.getWebRootPath()+"/"+url);
		oddAvatar.delete();
	}
}
